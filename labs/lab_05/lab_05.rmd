...
**A3.** The city with the highest loans in Berkeley County was Martinsburg. They industry in Martinsburg with the highest amount of loans was full service restaurants. 
...
#create new df just with project_county_name == ("BERKELEY")
berkeley_loans_with_naics<- wv_loans_with_naics_codes %>% 
...
berkeley_data_clean<- read_csv("data/berkely_loans_with_naics_clean.csv")
...
#use group_by in new df to group industries and see the highest count of industries in Martinsburg
martinsburg_data <- berkeley_data_clean %>% 
...
**A4.** My first hypotheses would be that the "full service restaurants" industry has the most loans because they have come up consistently in our data when we look at what industries received the most loans out of the entire data frame. I wouldn't be surprised if this WV town fell in line with a greater pattern. My second hypotheses would be that Martinsburg is the restaurant hub of the county, and being a heavily concentrated industry in the town, you would have the most loans. As seen in articles like [this](https://www.forbes.com › alangassman › 2021/12/28), I would believe my first hypotheses to be true. 
...
**A5.** COTTLE CHRISTI L LLC was approved with the highest loan amount of 280,434.00. Their loan was approved on 2/17/2021 and retained 94 jobs. Yes, the name the restaurant is known for is Kitzie's Restaurant. They have two locations. A third location in Inwood closed on 5/19/2020, after they received the loan.
```{r}

#use group_by in new df to group industries and see the highest count of industries in Martinsburg
businesses_top_city_top_industry <- berkeley_data_clean %>% 
  filter(title == "Full-Service Restaurants" & city == "Martinsburg") %>% 
  arrange(desc(amount))
```