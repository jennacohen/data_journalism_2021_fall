...
```{r}
international_migration <- read_csv("data/tabula-md-statistical-handbook.csv", skip=2, col_names=c("state_jurisdiction","total_2010_2011", "total_2011_2012", "total_2012_2013","total_2013_2014","total_2014_2015","total_2015_2016","total_2016_2017","total_2017_2018","total_2018_2019", "total_overall"))
```
...
**A1** The region that accounted for the largest percentage of international migration is the Suburban Washington Region, with 62.8%. The county/city with the largest percentage is Montgomery, with 36.8%.

```{r}
international_migration<- international_migration %>% 
  mutate(
    percentage_overall = (total_overall/198996)*100
  ) %>% 
  arrange(desc(percentage_overall))


```

**Q2** Write a sentence or two that describes the data you produced in A1. Try to convey the most important idea.
**A2** The suburban washington region had the largest percentage of migrants with 62.8% and Montgomery County, which falls within this region, is responsible for more than half of this total. 
...
**A3** The county with the largest percentage change was Calvert, with a 76.9% decline. The region with the biggest change was the Southern Maryland region, with a 58.5% decline.
```{r}
international_migration<- international_migration %>% 
  mutate(
    change_2018_2017 = ((total_2017_2018-total_2016_2017)/total_2016_2017)*100 
  )%>% 
  arrange(desc(change_2018_2017))

international_migration
```

**Q4** What's your best guess as to why these declines occurred, and in those area in particular?
**A4** I would infer that these declines occurred because of the election results in these regions. In the Calvert county gubernatorial election, 75% of residents voted Republican. Areas that are more republican are typically less open to migrants joining their communities. 
...
```{r}
domestic_migration <- read_csv("data/tabula-md-statistical-handbook-domestic.csv", col_names=c("state_jurisdiction","total_2010_2011", "total_2011_2012", "total_2012_2013","total_2013_2014","total_2014_2015","total_2015_2016","total_2016_2017","total_2017_2018","total_2018_2019", "total_overall"))
```
...
**A5** The Suburban Washington Region had the largest net decrease in domestic migration, with a net decrease of -107800.

```{r}
domestic_migration<- domestic_migration %>% 
  arrange(total_overall)
```
...
**A6** 14 regions & jurisdictions had net positive migration for all three years. 

```{r}
domestic_migration %>% 
  select(state_jurisdiction, total_2016_2017, total_2017_2018, total_2018_2019) %>% 
  filter(total_2016_2017 > 0 & total_2017_2018 > 0 & total_2018_2019 > 0)


```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** I would describe this data as standard as I don't see any one region standing out with having a major change in their net domestic migration. The majority of the jurisdictions are located on the Eastern Shore, and the two counties in the Southern Maryland Region, Howard and Fredrick, are have a positive net for the 3 years as well. 
