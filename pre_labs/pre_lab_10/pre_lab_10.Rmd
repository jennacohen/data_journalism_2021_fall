...
https://datawrapper.dwcdn.net/SySBm/1/
...
It's storing the name and count of all MD counties from the PPP data into a new dataframe. However in the data, there are a few anomolies that don't appear to be MD counties. 
...
**Answer** Describe what the code below is doing and the output.
It is filtering out any counties that seem to be listed by mistake. For example, there is one loan for Fairfax County, which is in NOVA. This ensures we are only dealing with MD jurisdictions. 
...
It joins together the state and county code to create a five digit string.
...
It shows all of the MD counties, including the few that were initially filtered out, with the count of loans, fips codes, and other columns that aren't of much use (ie. state name, state abbrieviated, state and county codes individually)
...
It much more zoomed in with a clear breakdown of colors. It also looks slightly tilted.
...
https://datawrapper.dwcdn.net/IcN4X/1/
