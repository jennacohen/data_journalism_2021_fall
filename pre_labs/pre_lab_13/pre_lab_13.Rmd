...
**Answer** Output has 51 rows and five columns. It has all 50 states plus DC.
```{r}
...
**Answer** Line of best fit shows a general trend from bottom left to top right- meaning more red states had a higher percentage of PPP loan recipients. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** It's a bit more scattered. There's less of a pattern among the plot points, though the line of best fit does slow a very slight positive slope. 
```{r}
...
**Answer** The business_receiving_ppp has a stronger correlation to pct_trump compared to ppe_amount_per_employee, which matches teh above graphs. I don't think the amount per employee has a particularly strong correlation at just .22
```{r}
reveal_data %>%
...
**Answer** An output of text stating data, alt hypothesis and sample estimates. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** A similar output of text as the previous task, however it has a much larger p value, making it less reliable. 
```{r}
...
**Answer** There's a difference of ~$300 of average of ppp per employee, and nearly a 9% difference between Clinton and Trump states that have businesses receiving ppp loans. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** There's a chunk of text that shows the two averages calculated above. The t-value is close enough to 1, meaning this finding is a bit more significant. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** There's a t-value of -3.2 and p-value of .002, making this finding significant. I think it would be a good idea to follow up this test on the following rounds of ppp loans to see if there is still a pattern that exists.
```{r}
