...
**Answer** It provides an overview of the BLS classification process, including things like history, structures, and industry codes. 
...
**Answer**You see the html file with various divs and style classes. 
...
**Answer** It shows two values, the head tag and the body tag.
...
**Answer** Columns: name, type and value ; four names: html, head, body, xml properties
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** <table> tags with a <span> to create the title. The contents of the table are nested in <tbody> </tbody>
...
**Answer** There are various tibbles with columns like reference month implements, sector and division.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** It still has the same column names, but now the names list 1,2,3,4,5,6. It also lists length of tibbles.
...
**Answer** It has columns conversion, reference month implemented and date released.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**It lists the sector along with the description of each sector number.
...
**Answer** It pulls the row into a tibble and shows the same sector number and description.
...
**Answer** It removes the last row that listed the source.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It discusses utilities sectors and workforce statistics.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** The top of an html document.
...
**Answer** It's an html document that lists contents of a table tag including head annd body.
...
**Answer** It lists employment numbers by the month
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It details information about the Mining, Quarrying, and Oil and Gas Extraction sector.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce statistices for the construction sector. 
...
**Answer** It repeats the lis of industry names with [1] in front of it. 
```{r}
...
**Answer** It lists the text in pairs of two on each row and the nnumbered list going from 1,3,5,7,9
```{r}
...
**Answer** It numbers them each out individually with a [1] in front.
```{r}
...
**Answer** It displays an error message that reads: Error in sector_list : object 'sector_list' not found
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** It appears the exact same as the codeblock above.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** It gives an error that sector_name is not found.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**it prints out every number from 1-10
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** It lists everything under section and descriptions, but some sector numbers list multiple numbers in their entry.
...
**Answer** It lists every number between 1-20
...
**Answer** It lists Employment, all employees (seasonally adjusted) for june of 2021:	586.7	
...
**Answer** it creates a table similar to the one above, just from fewer URLs in the code.
...
**Answer** It creates two more columns that list the sector url and the sector)xpath_id
...
**Answer** It removed the public administration row
...
**Answer** It produces each tibble, one for each sector and description with corresponding url and path.
...
**Answer** It lists all the rows out each in ""
```{r}
...
**Answer** It lists the data series alog with the employment statistics.
...
**Answer** It separates each data series into its on dataframe.
...
**Answer** It adds a description of the industry description, sector description and number of june 2021 employees. 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** It cleans up the df to contain each sector number, the sector name, and the jun_2021 employment number
...
**Answer** It combines all the rows from the previous codeblock into one dateframe. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It describes the Mining, Quarrying, and Oil and Gas Extraction industry.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It describes the Agriculture, Forestry, Fishing and Hunting statistices, specifically the unemployment rates.
...
**Answer** it lists NA for agriculture, forestry, fishing and hunting employee numbers 
