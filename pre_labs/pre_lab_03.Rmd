...
**Answer** It reads: Warning: One or more parsing issues, see `problems()` for details
...
**Answer** There are 87,927 problems when running the code.
...
**Answer** There was no alert or warning of parsing errors. The first thing you see are the number of rows and columns. 
...
**Answer** The "name" field is a character, while the "id" column is a number- even though as it mentions in the textbook, we need it as a character. 
```{r}
...
**Answer**: There were 192959 loans where the franchise name was n/a. There were 275 loans where the franchise name was Subway.
...
**Answer**: There are 135073 loans where the forgiveness amount is N/A.
...
**Answer**: There are 135073 loans that have a forgiveness date of N/A. That is the same amount of loans that had N/A for forgiveness amount.
...
**Answer**: There are 135073	loans where both the forgiveness amount and date is N/A, telling us that it's the same number from the codeblocks above. We can conclude that these 135073 loans were in fact forgiven, but we should check with the data custodian to verify before publishing. 
```{r}
ppp_maryland_loans %>%
...
**Answer** There are 0 loans for the month of October. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest amount was 10,000,000 dollars and the lowest amount was 6 dollars. I do find that suspicious because I don't see how a 6 dollar loan would be beneficial to someone's payroll.
```{r}
...
**Answer** There are 6 rows in this data set.
...
**Answer** It is a character field. <chr>
```{r}
...
**Answer** Wrong types, missing data and gaps in data. 
...
**Answer** It says that it's an invalid type because it is a character. I think it means that it can't perform math functions on something R doesnt recognize as a number. 
...
**Answer** There are 2. The business name is Stuart Jones LLC.
...
**Answer** Complete, now all the zip codes have 5 digits- 21012.
...
**Answer** There are still 2 mispellings.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer** One mispelling still remains.
